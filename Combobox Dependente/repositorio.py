...
class repoLocal:
            ...
            with connect.cursor() as curs:
                ...
                curs.execute(
                    ...
                    (
                        ...
                        AUTO_INCREMENT
                        PRIMARY
                        KEY, -- O RG da linha: o banco gera o número sozinho.
                        country
                        VARCHAR
                    (
                        255
                    ) NOT NULL, -- Coluna para o País (texto de até 255 letras).
                        state VARCHAR
                    (
                        255
                    ) NOT NULL, -- Coluna para o Estado.
                        city VARCHAR
                    (
                        255
                    ) NOT NULL, -- Coluna para a Cidade.
    ...
    def insertInitialDataIfBlank(self) -> None:
    ...
    def getQuantity(self) -> int:  # 1. "Vou criar uma função para me dizer a quantidade total (um número inteiro)."
        ...
        try:
            with con.cursor() as curs:  # 3. "Pego um funcionário (cursor) para anotar o que eu pedir."
                ...
                # 5. "Ele volta com a prancheta trazendo apenas a primeira linha encontrada (fetchone)."
                # Como COUNT(*) sempre retorna uma única linha com o total, fetchone() é suficiente.
                rows = curs.fetchone()
    ...
    def list(self, country: Optional[str] = None, state: Optional[str] = None, city: Optional[str] = None) -> List[
        ...
        if country:
            ...
            sql += " AND country = %s"
            # Também guardo o valor em 'params', pois ele será usado para preencher o %s com segurança.
        ...
        if state:
            ...
            sql += " AND state = %s"
            # Guardo novamente em params
            params.append(state)

        # 4. E finalizo com a cidade, seguindo a mesma regra.
        if city:
            sql += " AND city = %s"
            # Também guardo esse valor em 'params'
